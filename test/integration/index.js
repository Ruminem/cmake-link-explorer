...
async function runChecks() {
  ...
  // test/sample-project/build is regenerated by bootstrap.sh and gitignored, so
  // the log cannot live there. Stage the checked-in fixture instead, which is
  // also the only way to exercise the auto-load path: it looks for a file at a
  // fixed place rather than one the user picks.
  const timeProvider = api.getTimeProvider();
  const logs = path.join(__dirname, '..', 'ninja');
  const stagedLog = path.join(api.getBuildDirectory(), '.ninja_log');
  fs.copyFileSync(path.join(logs, 'mingw-incremental.ninja_log'), stagedLog);

  try {
    await check('a build log in the build directory opens by itself', async () => {
      await api.reload();
      const times = api.getTimes();
      assert.ok(times, 'reload did not pick up .ninja_log');
      const byName = new Map(Array.from(model.targets.values()).map((t) => [t.name, t]));
      assert.strictEqual(times.get(byName.get('engine').id).total, 217);
    });

    await check('the build time view renders real TreeItems', () => {
      const roots = timeProvider.getChildren();
      const total = timeProvider.getTreeItem(roots[0]);
      assert.ok(total.iconPath instanceof vscode.ThemeIcon, 'total icon is not a ThemeIcon');
      assert.strictEqual(total.description, '390 ms   ·   11.6 s of work across 18 steps');

      const targetGroup = roots.find((n) => n.group === 'targets');
      assert.ok(targetGroup, 'no "by target" group');
      const slowest = timeProvider.getChildren(targetGroup)[0];
      const item = timeProvider.getTreeItem(slowest);
      assert.strictEqual(item.label, 'math_utils');
      assert.ok(item.tooltip instanceof vscode.MarkdownString, 'tooltip is not a MarkdownString');
    });

    await check('the target tree grows a time column', () => {
      const roots = provider.getChildren();
      const item = (name) =>
        provider.getTreeItem(roots.find((n) => model.targets.get(n.id).name === name));
      // The map was closed above, so this is counts plus time and no size.
      const description = item('math_utils').description;
      assert.ok(/ 3\.2 s$/.test(description), 'expected the time last, got ' + description);
      assert.ok(!/[KM]?B$/.test(description), 'size column should be gone, got ' + description);
    });

    await check('comparing two build logs drops the time column', () => {
      const comparison = api.compareBuildLogs(path.join(logs, 'mingw-full.ninja_log'),
                                              path.join(logs, 'mingw-incremental.ninja_log'));
      assert.strictEqual(comparison.diff.total.delta, -136);
      assert.strictEqual(api.getTimes(), null, 'a diff cannot give one time per target');
    });

    await check('closing the build log sticks across a reload', async () => {
      await vscode.commands.executeCommand('cmakeLinkExplorer.closeBuildLog');
      assert.deepStrictEqual(timeProvider.getChildren(), []);
      assert.strictEqual(api.getTimes(), null, 'times should go away with the log');
      // The file is still sitting there. Without the dismissal being remembered
      // the next reload would reopen it and the command would look broken.
      await api.reload();
      assert.deepStrictEqual(timeProvider.getChildren(), []);
    });

    // Nothing here runs openBuildLog or diffBuildLogs: both open a quick pick,
    // and there is nobody to answer it.
    await check('a build tree with no ninja log is not an error', async () => {
      fs.unlinkSync(stagedLog);
      await api.reload();
      assert.strictEqual(api.getTimes(), null);
      assert.ok(api.getModel(), 'the target graph should still be loaded');
    });
  } finally {
    try { fs.unlinkSync(stagedLog); } catch (e) { /* already gone */ }
  }
