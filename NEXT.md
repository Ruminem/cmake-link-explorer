...
**여기까지 됨** — **0.2.2를 마켓플레이스에 올렸다.** 0.2.1 이후 쌓여 있던 인터페이스
수정 둘이 나갔다. 상태 표시줄이 돌지도 않은 빌드 트리 탐색 결과를 말하던 것, 그리고
저장에 실패한 링크 편집이 "저장됨"이라고 답하던 것. 태그는 `v0.2.2`.

**그 전** — 0.2.1을 올리면서 실제로 써 보고 조용히 틀리던 것들을 여섯 개 잡았다.
...
**PAT은 적어둔 만료일보다 먼저 죽는다.** 위에 2027-09-05로 적어 뒀는데 2026-09-07에
