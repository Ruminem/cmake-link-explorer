...
- **릴리스 노트는 전역 규칙(`~/.claude/CLAUDE.md`)을 따른다** — 태그마다 변경 내역을 적는다.
  **여기는 아직 그렇게 되어 있지 않다.** `release.yml`이 `--generate-notes`에 기대는데
  그것은 머지된 PR 목록으로 노트를 만들고, 이 프로젝트는 main에 직접 커밋하므로 나열할 PR이
  없어 노트가 비어 있다. 직전 태그부터의 `git log` 제목을 뽑아 붙이도록 고칠 것 —
  `vscode-neon-glow`의 `release.yml`에 같은 수정이 들어가 있으니 그대로 옮기면 된다.
  전체 이력이 필요하므로 체크아웃의 `fetch-depth: 0`도 같이 가야 한다.
